...
class Instructor(object):
    ...
    def train(self):
        ...
        for step in bar:
            ...
            if self.args.n_gpus>0:
                ...
                labels = labels.cuda()
            loss,logits = self.model(input_ids,attention_mask=mask_ids,labels=labels)[:2]
    ...
    def evaluate(self,report_path=None):
        ...
        for step in bar:
            ...
            assert logits_.shape[-1]==len(self.data_set.get_labels()), "logits error"
            preds = logits_.argmax(-1)
            y_true +=list(labels)
            y_pred += list(preds)
            assert len(labels) == len(preds), "labels shape: {}\ntags shape: {}\n input lens shape: {}".format(len(labels),len(preds))
    ...
    def prediction(self):
        ...
        df_out["label"] = [self.data_set.id2label[id] for id in all_preds]
    ...
    def load_model(self):
        ...
        if self.args.resume_model:
            self.model = BertForSequenceClassification.from_pretrained(self.args.resume_model_path,config=self.config)
        else:
