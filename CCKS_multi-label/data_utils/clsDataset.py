...
class ClsDataset(object):
    ...
    def transform(self,example):
        '''
        将example三元组数值化, 转换成bert模型需要的数据
        :param example: (id,content,type)组成的三元组
        :return: content_ids：tokenize并转换长id后的新闻文本
        attention_mask： 对应的文本和padding的mask
        labeling：[0,0,1,0,1,0,...,0] 29类 multi-label
        '''
        id = example[0]
        content = example[1]
        type = example[2]
        ...
        labeling = torch.tensor(labeling)
        return content_ids, attention_mask, labeling
